...
def fetch_srtm_30_zips(destination_folder: str, country_gpkg=None,
    ...
    for zipf in determine_srtm_30_zip(country_gpkg=country_gpkg):
        ...
        if force_download or not os.path.exists(zip_local_uri):
            download_url(zip_web_uri, zip_local_uri, chunk_size=1024*1024)
...
def unzip_tif_to_folder(zip_list: list, out_folder: str):
    ...
    existing_tif = glob.glob(out_folder + os.sep + '*.tif')

    for zipf in zip_list:
        arkiv = zipfile.ZipFile(zipf)
        arkiv.extractall(path=out_folder)

    unzipped_tif = glob.glob(out_folder + os.sep + '*.tif')
    unzipped_tif = [uzt for uzt in unzipped_tif if uzt not in existing_tif]

    return unzipped_tif


###############################################################################
def mosaic_rasters(input_raster_uri_list, output_raster_uri, nullvalue=-9999):
    """
    Joins list of rasters together into a new tif file.
    :param input_raster_uri_list: list of rasters to mosaic together
    :param output_raster_uri: output name of mosaicked raster
    ...
    mainmeta.update(nodata=nullvalue)
    with rasterio.open(output_raster_uri, 'w', **mainmeta) as outraster:
        outraster.write_band(1, mainlayer.astype(mainmeta['dtype']))


###############################################################################
def get_srtm30_for_country(download_folder: str, output_raster_uri: str,
    ...
    srtm30_tif_list = unzip_tif_to_folder(srtm30_zip_list, download_folder)
    mosaic_rasters(srtm30_tif_list, output_raster_uri, nullvalue=nullvalue)


###############################################################################
def get_srtm90_for_country(download_folder: str, output_raster_uri: str,
                           country_gpkg=None, nullvalue=-9999,
                           force_download=False):
    """
    Fetches SRTM 30m data for a given country
    ...
    mosaic_rasters(srtm90_tif_list, output_raster_uri, nullvalue=nullvalue)
